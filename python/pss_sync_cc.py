...
class pss_sync_cc(gr.basic_block):
    ...
    def general_work(self, input_items, output_items):
        ...
        if len(self.memory) >= self.fft_size*4 and not len(self.ssb) == self.ssb.maxlen:
            grids, nids, proc_samples = decode.sync_pss(received_data= self.memory,fft_size= self.fft_size, threshold= self.threshold,pss_candidates= self.can_pss_t)
            self.memory = self.memory[proc_samples:]
            for i in range(len(grids)):
                self.ssb.append([grids[i], self.i_ssb, nids[i]])
                self.i_ssb += 1
                self.i_ssb %= self.L_max


        n_out_items = min(len(output_items[1]), len(self.ssb))
        for i in range(n_out_items):
            ssb, i_ssb, nid2 = self.ssb.popleft()
            output_items[0][i][0] = nid2
            output_items[1][i][:] = ssb.flatten(order='F')
            output_items[2][i][0] = i_ssb
